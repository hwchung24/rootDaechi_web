...
export default function GilloApplyPage() {
  ...
  useEffect(() => {
    if (!config) router.replace("/gillo");
  }, [config, router]);
        ...
        >
          ...
          {status === "success" ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-700">현재 걱정되는 점 (선택)</label>
                <textarea
                  name="concerns"
                  value={form.concerns}
              ...
              <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center">
                ...
                <Link href="/gillo" className="text-center text-sm text-slate-500 hover:text-slate-700">
                  세팅 선택으로 돌아가기
                </Link>
              </div>
